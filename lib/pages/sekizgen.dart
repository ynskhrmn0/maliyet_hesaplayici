...
class Sekizgen extends StatefulWidget {
  const Sekizgen({super.key});

  @override
  State<Sekizgen> createState() => _SekizgenState();
}

class _SekizgenState extends State<Sekizgen> {
  var kenar;
  var yukseklik;
  var yuzey_alani;
  ...
  void hacim(int volume) {
    ...
    for (i = 0; i < volume;) {
      i = i + 0.1;
      a = i;
      h = volume / (2 * a * a * (1 + sqrt(2)));
      sonuc = (4 * a * a * (1 + sqrt(2))) + (8 * a * h);
      sonuclar.add(sonuc);
      sonuclar2.add(sonuc);
      kenarlar.add(a);
      yukseklikler.add(h);
